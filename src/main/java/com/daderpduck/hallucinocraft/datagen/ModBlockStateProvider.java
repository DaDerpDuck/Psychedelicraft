...
public class ModBlockStateProvider extends BlockStateProvider {
    public ModBlockStateProvider(PackOutput packOutput, ExistingFileHelper exFileHelper) {
        super(packOutput, Hallucinocraft.MOD_ID, exFileHelper);
    }
    ...
    protected void makeCrop(CropBlock block, String modelName, String textureName) {
        getVariantBuilder(block).forAllStates(blockState -> {
            ...
            models[0] = new ConfiguredModel(models().crop(modelName + block.getAge(blockState),
                    new ResourceLocation(Hallucinocraft.MOD_ID, "block/" + textureName + block.getAge(blockState))));
            return models;
        });
    }
    ...
    protected void makeCross(CropBlock block, String modelName, String textureName) {
        getVariantBuilder(block).forAllStates(blockState -> {
            ConfiguredModel[] models = new ConfiguredModel[1];
            models[0] = new ConfiguredModel(models().cross(modelName + block.getAge(blockState),
                    new ResourceLocation(Hallucinocraft.MOD_ID, "block/" + textureName + block.getAge(blockState))));
            return models;
        });
    }
